...
def c_kz_w_mkgrph(figname, title, timestep, dataset, ini, fin, flag):
    fig = plt.figure(figsize=[6,3])
    ax = fig.add_subplot(111)

    c_kz_ke_file = set_input_file(dataset, ini, fin, 'VINT', 'c_kz_ke', flag, 'grd')
    c_kz_ae_file = set_input_file(dataset, ini, fin, 'VINT', 'c_kz_ae', flag, 'grd')

    GM, NH, SH = plot_c_kz_w_ax(ax=ax, c_kz_ae_filename=c_kz_ae_file, c_kz_ke_filename=c_kz_ke_file, timestep=timestep, title=title)
...
def plot_c_kz_w_ax(ax, c_kz_ae_filename, c_kz_ke_filename, timestep, title=''):
    ...
    total = (get(c_kz_ae_filename, 'VINT', -90., 90., timestep) + get(c_kz_ke_filename, 'VINT', -90., 90., timestep))
    north = (get(c_kz_ae_filename, 'VINT',   0., 90., timestep) + get(c_kz_ke_filename, 'VINT',   0., 90., timestep))
    south = (get(c_kz_ae_filename, 'VINT', -90.,  0., timestep) + get(c_kz_ke_filename, 'VINT', -90.,  0., timestep))
