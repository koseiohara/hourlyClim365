...
def c_kz_w_mkgrph(figname, title, dataset, ini, fin, flag):

    ifileae = set_input_file(dataset, ini, fin, 'VINT', 'c_kz_ae', flag, 'grd')
    ifileke = set_input_file(dataset, ini, fin, 'VINT', 'c_kz_ke', flag, 'grd')
    DJFae, MAMae, JJAae, SONae = seasonal(filename=ifileae, hourstep=6, ny=145, leap=False)
    DJFke, MAMke, JJAke, SONke = seasonal(filename=ifileke, hourstep=6, ny=145, leap=False)
    ...
    DJFline, MAMline, JJAline, SONline = c_kz_w_ax(ax, DJF, MAM, JJA, SON, title, 1, 1.)
...
def c_kz_w_ax(ax, DJF, MAM, JJA, SON, title='', ydigit=0, ymajorstep=2.):
