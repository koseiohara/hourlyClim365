...
def ke_mkgrph(figname, title, dataset, ini, fin, flag):

    ifile = set_input_file(dataset, ini, fin, 'VINT', 'ke', flag, 'grd')
    DJF, MAM, JJA, SON = seasonal(filename=ifile, hourstep=6, ny=145, leap=False)

    fig = plt.figure(figsize=[6,3])
    ax = fig.add_subplot(111)
    DJFline, MAMline, JJAline, SONline = ke_ax(ax, DJF, MAM, JJA, SON, title, 1, 1.)
...
def ke_ax(ax, DJF, MAM, JJA, SON, title='', ydigit=0, ymajorstep=2.):
