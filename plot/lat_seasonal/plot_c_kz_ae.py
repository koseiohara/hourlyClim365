...
def c_kz_ae_mkgrph(figname, title, dataset, ini, fin, flag):

    ifile = set_input_file(dataset, ini, fin, 'VINT', 'c_kz_ae', flag, 'grd')
    DJF, MAM, JJA, SON = seasonal(filename=ifile, hourstep=6, ny=145, leap=False)

    fig = plt.figure(figsize=[6,3])
    ax = fig.add_subplot(111)
    DJFline, MAMline, JJAline, SONline = c_kz_ae_ax(ax, DJF, MAM, JJA, SON, title, 1, 1.)
...
def c_kz_ae_ax(ax, DJF, MAM, JJA, SON, title='', ydigit=0, ymajorstep=2.):
