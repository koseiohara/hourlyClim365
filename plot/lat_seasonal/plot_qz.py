...
def qz_mkgrph(figname, title, dataset, ini, fin, flag):

    ifiletot = set_input_file(dataset, ini, fin, 'VINT',       'qz', flag, 'grd')
    ifiletts = set_input_file(dataset, ini, fin, 'VINT', 'ttswr_qz', flag, 'grd')
    ifilettl = set_input_file(dataset, ini, fin, 'VINT', 'ttlwr_qz', flag, 'grd')
    ifilelrg = set_input_file(dataset, ini, fin, 'VINT', 'lrghr_qz', flag, 'grd')
    ifilecnv = set_input_file(dataset, ini, fin, 'VINT', 'cnvhr_qz', flag, 'grd')
    ifilevdf = set_input_file(dataset, ini, fin, 'VINT', 'vdfhr_qz', flag, 'grd')
    ...
    DJFline, MAMline, JJAline, SONline = qz_ax(total, DJFtot, MAMtot, JJAtot, SONtot, 'Total', 1, 10.)
    qz_ax(ttswr, DJFtts, MAMtts, JJAtts, SONtts, 'Short Wave Radiation'    , 1, 10.)
    qz_ax(ttlwr, DJFttl, MAMttl, JJAttl, SONttl, 'Long Wave Radiation'     , 1, 10.)
    qz_ax(lrghr, DJFlrg, MAMlrg, JJAlrg, SONlrg, 'Large Scale Condensation', 1, 10.)
    qz_ax(cnvhr, DJFcnv, MAMcnv, JJAcnv, SONcnv, 'Convective Heating'      , 1, 10.)
    qz_ax(vdfhr, DJFvdf, MAMvdf, JJAvdf, SONvdf, 'Vertical Diffusion'      , 1, 10.)
