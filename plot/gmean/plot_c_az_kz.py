...
def c_az_kz_mkgrph(figname, title, timestep, dataset, ini, fin, flag):
    fig = plt.figure(figsize=[6,3])
    ax = fig.add_subplot(111)

    c_az_kz_file = set_input_file(dataset, ini, fin, 'VINT', 'c_az_kz', flag, 'grd')

    GM, NH, SH = plot_c_az_kz_ax(ax=ax, filename=c_az_kz_file, timestep=timestep, title=title)
...
def plot_c_az_kz_ax(ax, filename, timestep, title=''):
