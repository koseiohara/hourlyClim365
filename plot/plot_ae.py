...
def ae_mkgrph(figname, title, timestep, dataset, ini, fin, flag):
    fig = plt.figure(figsize=[6,3])
    ax = fig.add_subplot(111)

    ae_file = set_input_file(dataset, ini, fin, 'VINT', 'ae', flag, 'grd')
                              
    GM, NH, SH = plot_ae_ax(ax=ax, filename=ae_file, timestep=timestep, title=title)
...
def plot_ae_ax(ax, filename, timestep, title=''):
