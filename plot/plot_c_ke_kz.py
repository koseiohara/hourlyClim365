...
def c_ke_kz_mkgrph(figname, title, timestep, dataset, ini, fin, flag):
    fig = plt.figure(figsize=[6,3])
    ax = fig.add_subplot(111)

    c_kz_ke_file = set_input_file(dataset, ini, fin, 'VINT', 'c_kz_ke', flag, 'grd')

    GM, NH, SH = plot_c_ke_kz_ax(ax=ax, filename=c_kz_ke_file, timestep=timestep, title=title)
...
def plot_c_ke_kz_ax(ax, filename, timestep, title=''):
