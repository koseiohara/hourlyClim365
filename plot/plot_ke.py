...
def ke_mkgrph(figname, title, timestep, dataset, ini, fin, flag):
    fig = plt.figure(figsize=[6,3])
    ax = fig.add_subplot(111)

    ke_file = set_input_file(dataset, ini, fin, 'VINT', 'ke', flag, 'grd')

    GM, NH, SH = plot_ke_ax(ax=ax, filename=ke_file, timestep=timestep, title=title)
...
def plot_ke_ax(ax, filename, timestep, title=''):
