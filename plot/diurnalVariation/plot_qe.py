...
def plot_qe(sw_file, lw_file, lr_file, cn_file, vd_file, figname, title):
    ...
    djf, mam, jja, son = qe_ax(tt, xlab, tt_djf, tt_mam, tt_jja, tt_son, 'Total')
    qe_ax(sw, xlab, sw_djf, sw_mam, sw_jja, sw_son, 'Short Wave Radiation')
    qe_ax(lw, xlab, lw_djf, lw_mam, lw_jja, lw_son, 'Long Wave Radiation')
    qe_ax(lr, xlab, lr_djf, lr_mam, lr_jja, lr_son, 'Large Scale Condensation')
    qe_ax(cn, xlab, cn_djf, cn_mam, cn_jja, cn_son, 'Convective Heating')
    qe_ax(vd, xlab, vd_djf, vd_mam, vd_jja, vd_son, 'Vertical Diffusion')
...
def qe_ax(ax, xlab, DJF, MAM, JJA, SON, title=''):
