...
def w_mkgrph(figname, title, timestep, dataset, ini, fin, flag):
    fig = plt.figure(figsize=[6,3])
    ax = fig.add_subplot(111)

    ke_file = set_input_file(dataset, ini, fin, 'VINT', 'ke', flag, 'grd')
    ae_file = set_input_file(dataset, ini, fin, 'VINT', 'ae', flag, 'grd')

    GM, NH, SH = plot_w_ax(ax=ax, ae_filename=ae_file, ke_filename=ke_file, timestep=timestep, title=title)
...
def plot_w_ax(ax, ae_filename, ke_filename, timestep, title=''):
    ...
    total = (get(ae_filename, 'VINT', -90., 90., timestep) + get(ke_filename, 'VINT', -90., 90., timestep)) * 1.E-6
    north = (get(ae_filename, 'VINT',   0., 90., timestep) + get(ke_filename, 'VINT',   0., 90., timestep)) * 1.E-6
    south = (get(ae_filename, 'VINT', -90.,  0., timestep) + get(ke_filename, 'VINT', -90.,  0., timestep)) * 1.E-6
